...
#[view]
pub fn view(model: &Model, context: &Context, page_id: Option<PageId>) -> Node<Msg> {
    div![
        C!["route-content"],
        s()
        ...
        div![
            ...
            nav_content_container(&model.video_groups),
        ]
    ]
}
...
#[view]
fn vertical_nav_bar(root_base_url: &Url, page_id: Option<PageId>) -> Node<Msg> {
    nav![
...
#[view]
fn vertical_nav_buttons(root_base_url: &Url, page_id: Option<PageId>) -> Vec<Node<Msg>> {
    vec![
        { 
            let active = page_id == Some(PageId::Board);
            vertical_nav_button(
                "Board", 
                active,
        ...
        {
            let active = page_id == Some(PageId::Discover);
            vertical_nav_button(
                "Discover", 
                active,
        ...
        {
            let active = page_id == Some(PageId::Addons);
            vertical_nav_button(
                "Addons", 
                active,
...
#[view]
fn vertical_nav_button(title: &str, active: bool, href: Option<String>, margin_top: bool, icon: Node<Msg>) -> Node<Msg> {
    a![
